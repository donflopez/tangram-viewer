...
Carto.generateVizUri = function (url, low) {
  let com = url.indexOf('.com') + 4,
      lan = url.indexOf('.lan') + 4,
      domain = com !== 3 ? url.substring(0, com) : url.substring( 0, lan) + ':3000';
  return domain + '/api/v3/viz/' + /\/([\w*\-A-z0-9]*\-[A-z0-9]*)\//g.exec( url )[ 1 ].replace(/-/g, low ? '_' : '-') + '/viz.json';
}
...
Carto.generateJSONPUri = function (vizJSON) {
  let source = Carto.generateSource(vizJSON.datasource, true);
